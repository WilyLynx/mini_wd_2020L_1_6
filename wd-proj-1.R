...
grants_mapped2 <- grants_mapped2 %>% 
  mutate(subpanel = panel, panel =substr(panel,0,2))

grants_mapped2 <- grants_mapped2 %>% 
  mutate( descriptors= strsplit(descriptors,"\\|"),) %>% unnest(c(descriptors)) 

mean_budget_per_month <- grants_mapped2 %>% 
...
data_global_mean_contest_budget %>%  
  ggplot(aes(x=number,y=mean_budget/1000,color=panel,label=contest , shape=contest)) +
  geom_point(size=10, stroke=2.5) + 
  geom_text(size=3,color="black",nudge_y = -0.1) +
  scale_shape_manual(values=c(15, 10, 17,18,19,2,7,22,23)) +
  scale_y_log10(labels = label_comma(suffix = " tys. zł"))+
  scale_x_log10(limits=c(29,3100))+
